...
public enum GestureActionKind
{
    ...
    VolumeUp,
    VolumeDown,
    /// <summary>滑动整体动作：音量调节（上滑=音量+ / 下滑=音量-）。官方 App 的滑动选项之一。</summary>
    VolumeControl,
    /// <summary>滑动整体动作：歌曲切换（上滑=下一曲 / 下滑=上一曲）。官方 App 的滑动选项之一。</summary>
    SongSwitch,
    Translate,
    QuickAttention,
...
/// <summary>长按「切换噪声控制」循环集合里的一个可勾选模式（如 降噪/自适应/通透/关闭）。</summary>
public sealed record LongPressCycleOption(NoiseMode Mode, string DisplayKey, bool IsSelected);
...
public sealed record GestureEntry(
    ...
    /// <summary>MultiCheckbox 形态下的勾选集合（null 表示非多选行）。</summary>
    IReadOnlyList<LongPressCycleOption>? CycleOptions = null);
...
public static class GestureDisplay
{
    public static string KeyFor(GestureActionKind kind) => kind switch
    {
        ...
        GestureActionKind.VolumeControl => "DeviceInfo_VolumeControl",
