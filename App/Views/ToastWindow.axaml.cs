...
public partial class ToastWindow : Window
{
    ...
    private static (int Level, bool Charging)? TryGet(BatteryState battery, string side) => side switch
    {
        "L" => battery.Left is int l ? (l, battery.LeftCharging) : null,
        "R" => battery.Right is int r ? (r, battery.RightCharging) : null,
        "C" => battery.Case is int c ? (c, battery.CaseCharging) : null,
        _ => null,
    };

    public static async Task ShowAsync(CoreState? state, string deviceName,
        ToastType type = ToastType.Battery, int durationMs = 5000)
    {
        ...
        else
        {
            ...
            if (state != null)
            {
                SetBat(toast.LeftPct, toast.LeftBolt, MergeCharge(TryGet(state.Battery, "L"), state.LeftWearing));
                SetBat(toast.RightPct, toast.RightBolt, MergeCharge(TryGet(state.Battery, "R"), state.RightWearing));
